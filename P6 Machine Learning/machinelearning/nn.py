...
class Graph(object):
    ...
    def add(self, node):
        ...
        self.dic[node] = [self.get_output(node), np.zeros_like(self.get_output(node))]
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        self.dic[loss_node][1] = 1.
        for n in self.nodes[::-1]:
            gs = n.backward(self.get_inputs(n), self.dic[n][1])
            p = n.get_parents()
            if p:
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient, np.dot(np.ones_like(gradient).T, gradient)[0]]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        copy_grad = np.copy(gradient)
        copy_grad[np.where(inputs[0] < 0)] = 0
        return [copy_grad]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return (0.5 * ((inputs[1] - inputs[0]) ** 2)).mean()

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        grad1 = np.array((inputs[0] - inputs[1]) * gradient / len(inputs[0]))
        grad2 = np.array((inputs[1] - inputs[0]) * gradient / len(inputs[1]))
        return [grad1, grad2]
