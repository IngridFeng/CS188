...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    maximum = max(self.computeQValueFromValues(state, action), maximum)
                    curr[state] = maximum
            self.values = curr
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next_state, prob in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            discounted_val = self.values[next_state] * self.discount
            q_val += prob * (discounted_val + reward)
        return q_val
    ...
    def computeActionFromValues(self, state):
        ...
        for action in self.mdp.getPossibleActions(state):
            ...
            if q_val > decision[0]:
                decision[0], decision[1] = q_val, action
        return decision[1]
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        "*** YOUR CODE HERE ***"
        pri_que = util.PriorityQueue()
        pred = {}

        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                action = self.computeActionFromValues(state)
                maximum = self.computeQValueFromValues(state, action)
                diff = abs(maximum - self.values[state])
                pri_que.update(state, -diff)
                for action in self.mdp.getPossibleActions(state):
                    for next_state, prob in self.mdp.getTransitionStatesAndProbs(state, action):
