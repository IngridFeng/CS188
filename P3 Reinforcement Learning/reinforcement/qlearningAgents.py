...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for a in legalActions:
            q_val = self.q_val[(state, a)]
            if q_val > maximum:
                maximum, action = q_val, a
        return action
    ...
    def update(self, state, action, nextState, reward):
        ...
        reward_val = self.alpha * reward
        nextState_val = self.alpha * self.discount * self.getValue(nextState)
        self.q_val[(state, action)] = old + reward_val + nextState_val
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            # print "numTraining: {0}".format(self.numTraining)
            pass
