    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        dist, ghostPos = float("inf"), None
        for d in livingGhostPositionDistributions:
            bufferPos = d.argMax()
            bufferDist = self.distancer.getDistance(bufferPos, pacmanPosition)
            dist = min(bufferDist, dist)
            if bufferDist == dist:
                ghostPos = bufferPos

        dist, chosenAction = float("inf"), None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            bufferDist = self.distancer.getDistance(ghostPos, successorPosition)
            dist = min(bufferDist, dist)
            if bufferDist == dist:
                chosenAction = action
        return chosenAction
